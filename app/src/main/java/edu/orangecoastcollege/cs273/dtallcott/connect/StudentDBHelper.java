...
class StudentDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String createDatabase = "CREATE TABLE " + DATABASE_TABLE + "(_id PRIMARY KEY AUTOINCREMENT, "
    ...
    public List<Student> getStudent(Student dummy)
    {
        ...
        Cursor cursor = db.query(DATABASE_TABLE,new String[]{FIELD_FIRST_NAME, FIELD_LAST_NAME, FIELD_COURSES, FIELD_IMAGE_NAME},where
                ,null,null,null,null);

        if(cursor.moveToFirst())
        {
            do{
                Student newStudent = new Student(cursor.getColumnName(0), cursor.getColumnName(1),
                        coursesStringToList(cursor.getColumnName(2)));
